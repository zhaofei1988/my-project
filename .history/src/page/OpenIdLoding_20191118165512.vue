...
export default {
  ...
  created() {
    if (window.localStorage && window.localStorage.henglianOpen) {
      this.$router.replace({
        path: "/index",
        query: { phone: window.localStorage.henglianOpen }
      });
      return null;
    }
      ...
      httpcc
        ...
        .then(data => {
          if (data.code == 2) {
            ...
            window.localStorage.setItem("henglianPhong",data.phone)
            this.$router.replace({
              path: "/index",
            ...
            MessageBox.alert(
              "您不是司机，或者您还没有备注，如果您没有备注请找相关人员备注",
              "提示"
            );
          }
        })
